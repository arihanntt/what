...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      timer: ReturnType<typeof setTimeout> | null = null; container: HTMLElement | null = null; docTarget: Document | null = null
      listenerTarget: Window | null = null; isHoverInside = false; hasUserControl = false; isAutoActive = false
      autoIntensity2 = 2.0; takeoverActive = false; takeoverStartTime = 0; takeoverDuration2 = 0.25
      ...
      update() {
        if (this.takeoverActive) {
          const t = (performance.now() - this.takeoverStartTime) / (this.takeoverDuration2 * 1000)
        ...
        if (this.isAutoActive && !this.takeoverActive) this.diff.multiplyScalar(this.autoIntensity2)
      }
    }
    const Mouse = new MouseClass()

    class AutoDriver {
      mouse: MouseClass; manager: any; enabled: boolean; speed: number; resumeDelay: number
      rampDurationMs: number; active = false; current = new THREE.Vector2(); target = new THREE.Vector2()
      lastTime = performance.now(); activationTime = 0; margin = 0.2; _tmpDir = new THREE.Vector2()
    ...
    class ShaderPass {
      ...
      plane: THREE.Mesh | null = null; line: THREE.LineSegments | null = null
    ...
    class ExternalForce extends ShaderPass {
      ...
      init(p: any) {
        super.init()
        const m = new THREE.RawShaderMaterial({ vertexShader: mouse_vert, fragmentShader: externalForce_frag, blending: THREE.AdditiveBlending, depthWrite: false, uniforms: { px: { value: p.cellScale }, force: { value: new THREE.Vector2(0,0) }, center: { value: new THREE.Vector2(0,0) }, scale: { value: new THREE.Vector2(p.cursor_size, p.cursor_size) } } })
        this.mouse = new THREE.Mesh(new THREE.PlaneGeometry(1,1), m); this.scene!.add(this.mouse)
      }
      update(p: any) {
        const fx = (Mouse.diff.x/2)*p.mouse_force; const fy = (Mouse.diff.y/2)*p.mouse_force
        const csx = p.cursor_size*p.cellScale.x; const csy = p.cursor_size*p.cellScale.y
        const cx = Math.min(Math.max(Mouse.coords.x, -1+csx+p.cellScale.x*2), 1-csx-p.cellScale.x*2)
        const cy = Math.min(Math.max(Mouse.coords.y, -1+csy+p.cellScale.y*2), 1-csy-p.cellScale.y*2)
        const u = (this.mouse!.material as THREE.RawShaderMaterial).uniforms
        u.force.value.set(fx,fy); u.center.value.set(cx,cy); u.scale.value.set(p.cursor_size,p.cursor_size); super.update()
      }
    }
    ...
    class Simulation {
      ...
      advection: Advection | null = null; externalForce: ExternalForce | null = null; viscous2: Viscous | null = null
      divergence2: Divergence | null = null; poisson2: Poisson | null = null; pressure2: Pressure | null = null
      ...
      createShaderPass() {
        ...
        this.viscous2 = new Viscous({ cellScale:this.cellScale, boundarySpace:this.boundarySpace, viscous:this.options.viscous, src:this.fbos.vel_1, dst:this.fbos.vel_viscous1, dst_:this.fbos.vel_viscous0, dt:this.options.dt })
        this.divergence2 = new Divergence({ cellScale:this.cellScale, boundarySpace:this.boundarySpace, src:this.fbos.vel_viscous0, dst:this.fbos.div, dt:this.options.dt })
        this.poisson2 = new Poisson({ cellScale:this.cellScale, boundarySpace:this.boundarySpace, src:this.fbos.div, dst:this.fbos.pressure_1, dst_:this.fbos.pressure_0 })
        this.pressure2 = new Pressure({ cellScale:this.cellScale, boundarySpace:this.boundarySpace, src_p:this.fbos.pressure_0, src_v:this.fbos.vel_viscous0, dst:this.fbos.vel_0, dt:this.options.dt })
      }
      calcSize() {
      ...
      update() {
        ...
        if (this.options.isViscous) vel = this.viscous2!.update({ viscous:this.options.viscous, iterations:this.options.iterations_viscous, dt:this.options.dt })
        this.divergence2!.update({ vel })
        const pressure = this.poisson2!.update({ iterations:this.options.iterations_poisson })
        this.pressure2!.update({ vel, pressure })
      }
    }
    ...
    class WebGLManager {
      props: any; output: Output | null = null; autoDriver: AutoDriver; lastUserInteraction: number; running = false
      _loop: () => void; _resize: () => void; _onVisibility: () => void
      constructor(p: any) {
        this.props = p; Common.init(p.$wrapper); Mouse.init(p.$wrapper)
        Mouse.autoIntensity2 = p.autoIntensity; Mouse.takeoverDuration2 = p.takeoverDuration
